...
#[test]
fn stale_dirty_confirm_save_and_discard_report_visible_feedback() {
    let mut ctx = ctx_or_skip!();
    let handle = (&mut ctx as *mut MuiContext) as usize as i64;

    assert_eq!(crate::abi::mui_dirty_confirm_save(handle), -1);
    let toast = ctx.toasts.toasts().last().unwrap();
    assert_eq!(toast.kind, crate::toast::Kind::Info);
    assert_eq!(toast.message, "No unsaved changes confirmation open");

    assert_eq!(crate::abi::mui_dirty_confirm_discard(handle), -1);
    let toast = ctx.toasts.toasts().last().unwrap();
    assert_eq!(toast.kind, crate::toast::Kind::Info);
    assert_eq!(toast.message, "No unsaved changes confirmation open");
}
...
fn tree_abi_scan_toggle_and_open_row() {
    ...
    assert_eq!(mui_tree_open_row(handle, 99), -1);
    let toast = ctx.toasts.toasts().last().unwrap();
    assert_eq!(toast.kind, crate::toast::Kind::Info);
    assert_eq!(toast.message, "Explorer row no longer listed");
...
fn web_run_rejects_missing_active_path_before_spawn() {
    ...
    assert_eq!(
        toast.message,
        "Run in Browser: failed to start: target missing: main.mty (see panel)"
    );
...
fn test_run_spawn_failure_names_target_and_command() {
    let _g = crate::settings::TEST_LOCK
        ...
        .unwrap_or_else(|e| e.into_inner());
    let mut ctx = ctx_or_skip!();
    let root = std::env::temp_dir().join(format!("mui_test_spawn_fail_{}", std::process::id()));
    std::fs::create_dir_all(&root).unwrap();
    let file = root.join("main.mty");
    let missing_mty = root.join("missing-mty.exe");
    ...
    assert!(row.message.contains("failed to run"));
...
fn test_result_open_misses_report_visible_feedback() {
    ...
    assert_eq!(
        toast.message,
        "Test result row has no file target: test_addition"
    );
    assert_eq!(crate::testabi::mui_test_click_tab(handle), -1);
...
fn terminal_input_acknowledges_closed_state_without_requiring_pty_spawn() {
    ...
    crate::abi::mui_term_scroll(handle, 1);
    let toast = ctx.toasts.toasts().last().unwrap();
    assert_eq!(toast.kind, crate::toast::Kind::Warn);
    assert_eq!(toast.message, "Terminal is not open");
...
#[test]
fn active_file_rename_rejects_directory_source() {
    let mut ctx = ctx_or_skip!();
    let root = std::env::temp_dir().join("mui_active_file_rename_directory_source");
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    let old = root.join("old.mty");
    let new = root.join("new.mty");
    ...
    let toast = ctx.toasts.toasts().last().unwrap();
    assert_eq!(toast.kind, crate::toast::Kind::Error);
    assert_eq!(toast.message, "Rename failed: old.mty: not a file");

    let _ = std::fs::remove_dir_all(&root);
}

#[test]
fn active_file_rename_rejects_directory_destination() {
    let mut ctx = ctx_or_skip!();
    let root = std::env::temp_dir().join("mui_active_file_rename_directory_destination");
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    let old = root.join("old.mty");
    let new = root.join("new.mty");
    ...
    let toast = ctx.toasts.toasts().last().unwrap();
    assert_eq!(toast.kind, crate::toast::Kind::Error);
    assert_eq!(toast.message, "Rename failed: new.mty: not a file");
...
fn active_file_delete_rejects_directory_target() {
    ...
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    let dir = root.join("not_a_file.mty");
    std::fs::create_dir_all(&dir).unwrap();
    ctx.workspace.set_root(root.clone());
    ctx.tree.set_root(root.clone());
    ctx.tree.refresh();
...
#[test]
fn debug_start_rejects_read_only_binary_preview_before_spawn() {
    let mut ctx = ctx_or_skip!();
    let root = std::env::temp_dir().join(format!(
        "mui_debug_read_only_target_{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
...
#[test]
fn debug_start_spawn_failure_names_target_and_command() {
    let _g = crate::settings::TEST_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    let mut ctx = ctx_or_skip!();
    let root = std::env::temp_dir().join(format!("mui_debug_spawn_fail_{}", std::process::id()));
    std::fs::create_dir_all(&root).unwrap();
    let file = root.join("main.mty");
    let missing_mty = root.join("missing-mty.exe");
    std::fs::write(&file, b"fn main() {}\n").unwrap();
...
#[test]
fn debug_restart_rejects_missing_previous_target_before_spawn() {
    let mut ctx = ctx_or_skip!();
    let root = std::env::temp_dir().join(format!(
        "mui_debug_restart_missing_{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    let file = root.join("again.mty");
    std::fs::write(&file, b"fn main() {}\n").unwrap();
    ctx.dbg.seed_demo(&file.to_string_lossy());
    std::fs::remove_file(&file).unwrap();
    ctx.sidebar_visible = false;
    ctx.active_panel = crate::PANEL_EXPLORER;
    let handle = (&mut ctx as *mut MuiContext) as usize as i64;
    ...
    assert!(
        toast
            .message
            .starts_with("Debug restart failed: again.mty via "),
        "{}",
        toast.message
    );
    assert!(
...
#[test]
fn debug_restart_rejects_directory_previous_target_before_spawn() {
    let mut ctx = ctx_or_skip!();
    let root = std::env::temp_dir().join(format!("mui_debug_restart_dir_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    let file = root.join("again.mty");
    std::fs::write(&file, b"fn main() {}\n").unwrap();
    ...
    assert!(
        toast.message.ends_with("dap: target is not a file: again.mty"),
        "{}",
        toast.message
    );
    let console = ctx.dbg.console_line(0).expect("debug failure console line");
    ...
    assert_eq!(
        console.text,
        "debug: failed to start adapter: target is not a file: again.mty"
    );
...
#[test]
fn navigation_requests_report_unavailable_language_server() {
    let _guard = crate::settings::TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let old_config_dir = std::env::var_os("MUI_CONFIG_DIR");
    let root = std::env::temp_dir().join(format!(
        "mui_nav_lsp_unavailable_{}",
        std::process::id()
    ));
    ...
    assert_eq!(crate::mui_hover_request(h, 0, 4), 0);
    let toast = ctx.toasts.toasts().last().unwrap();
    assert_eq!(toast.kind, crate::toast::Kind::Warn);
    assert_eq!(toast.message, expected);

    assert_eq!(crate::mui_def_request(h, 0, 4), 0);
    let toast = ctx.toasts.toasts().last().unwrap();
    assert_eq!(toast.kind, crate::toast::Kind::Warn);
    assert_eq!(toast.message, expected);

    assert_eq!(crate::abi::mui_sig_request(h, 0, 8), 0);
    let toast = ctx.toasts.toasts().last().unwrap();
    assert_eq!(toast.kind, crate::toast::Kind::Warn);
    assert_eq!(toast.message, expected);
...
#[test]
fn save_as_dialog_rejects_directory_target_as_not_file() {
    use crate::{mui_active_has_path, mui_ed_dirty, mui_save_as_dialog};
    ...
    let h = (&mut ctx as *mut MuiContext) as usize as i64;
    let root = std::env::temp_dir().join(format!(
        "mui_save_as_dialog_directory_target_{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(&root).unwrap();
    let target = root.join("folder.mty");
    std::fs::create_dir_all(&target).unwrap();
    ...
    let toast = ctx.toasts.toasts().last().unwrap();
    assert_eq!(toast.kind, crate::toast::Kind::Error);
    assert_eq!(toast.message, "Save failed: folder.mty: not a file");
...
fn plain_save_skips_conflicting_dirty_duplicate_tab() {
    ...
    assert_eq!(
        toast.message,
        "Save skipped: same.mty has duplicate unsaved edits"
    );
