...
mod tests {
    ...
    fn file_descriptions_name_read_only_active_target() {
        ...
        assert_eq!(
            engine.contextual_desc(&ctx, CMD_RUN_FILE, "base").as_ref(),
            "asset.bin is read-only in the text editor"
        );
        assert_eq!(
            engine
        ...
        assert_eq!(
            engine.contextual_desc(&ctx, CMD_INLINE_AI_ASK, "base").as_ref(),
            "asset.bin is read-only in the text editor"
        );
        assert_eq!(
            engine
                .contextual_desc(&ctx, CMD_FORCE_GHOST_COMPLETION, "base")
                .as_ref(),
            "asset.bin is read-only in the text editor"
        );
    ...
    fn save_description_reports_dirty_duplicate_tabs() {
        ...
        ctx.tabs.active_model_mut().set_text_preserving_cursor("active dirty\n");
