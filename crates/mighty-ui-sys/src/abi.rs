...
fn lsp_semantic_labels_with_notice(
    lang: Language,
    path: &std::path::Path,
    source: &str,
    line: u32,
    ...
    if lang == Language::Mighty {
        return (
            crate::completion::lsp::semantic_labels(path, source, line, col),
            None,
        );
    }
    ...
    if std::env::var_os("MUI_LIGHTBULB_AUTOOPEN").is_some() {
        if let Some(ctx) = unsafe { ctx(handle) } {
            ...
            let line = std::env::var("MUI_LIGHTBULB_AUTOOPEN")
                .ok()
                .and_then(|v| crate::parse_signed_decimal_i32_token(v.trim()))
                .filter(|v| *v >= 0)
                .unwrap_or(0);
...
";
    ...
    if let Some(seed) = std::env::var_os("MUI_SETTINGS_AUTOOPEN") {
        if let Some(ctx) = unsafe { ctx(handle) } {
            ...
            if let Some(row) = crate::parse_signed_decimal_i32_token(v.trim()) {
...
pub(crate) fn file_target_name(path: &std::path::Path) -> String {
    path.file_name()
        .map(|s| s.to_string_lossy().into_owned())
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| path.to_string_lossy().into_owned())
