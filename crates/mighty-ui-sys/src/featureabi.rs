...
fn stale_run_target_reason(path: &std::path::Path) -> Option<String> {
    let path_text = path.to_string_lossy();
    let label = run_path_label(&path_text).to_string();
    match run_target_kind(path) {
        RunTargetKind::File => None,
        RunTargetKind::Missing => Some(format!("target missing: {label}")),
        RunTargetKind::NotFile => Some(format!("target is not a file: {label}")),
    }
}
...
pub extern "C" fn mui_run_start(handle: i64) -> i32 {
    ...
    if let Some(reason) = stale_run_target_reason(&path) {
        ...
        ctx.push_toast(
            ...
            run_start_failed_message(&path, toast_reason.as_deref()),
        );
        crate::abi::trace(&format!("run_start stale target={}", path.display()));
        return 0;
    }
