...
inputs, snippet tab-stops, code-action menus, quick-fix lightbulbs, inline peek
cards, and breadcrumb dropdowns are all computed from the current buffer,
current cursor, or current file folder. If they survive an active-file
transition, they can point at the previous tab while the status bar and editor
show a new file.
