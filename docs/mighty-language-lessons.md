...
_Last updated: 2026-05-29 (LIVE EDITING via a shim-side authoritative text model — the L28 workaround; "Ember Graphite" visual redesign + bundled JetBrains Mono. Prior: L28 `v = v.push(x)` capture-rebind NO-OP under native `mty build`; command palette shim-side registry; L27.)_
