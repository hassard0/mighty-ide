...
_Last updated: 2026-06-06 (Config text file budget - L1211; Reload oversized file budget - L1210; Peek preview file budget - L1209; Project search file budget - L1208; Oversized file open preview - L1207; Native dialog output cap - L1206; Git blame output cap - L1205; Source Control read output cap - L1204; Web Playground build output cap - L1203; New Project output cap - L1202; Formatter output cap - L1201; Diff hunk apply output cap - L1200; Inline diff output cap - L1199; Agents inspect output cap - L1198; Diagnostics output cap - L1197; Source Control git output cap - L1196; Clipboard read size budget - L1195; Mighty LSP output cap fail-closed - L1194; Generic LSP output cap fail-closed - L1193; DAP frame size cap - L1192; Git blame timezone token boundaries - L1191; Run output clickable location token boundaries - L1190; Compiler diagnostic location token boundaries - L1189; Git blame timestamp token boundaries - L1188; Markdown ordered-list fallback numbering - L1187; Markdown ordered-list overflow resilience - L1186; Web Playground port override token boundaries - L1185; Quick Open and Testing numeric token boundaries - L1184; Persisted numeric preference token boundaries - L1183; Shortcut override numeric token boundaries - L1182; Terminal numeric token boundaries - L1181; Headless environment numeric token boundaries - L1180; Completion numeric token boundaries - L1179; Run output location label token boundaries - L1178; Snippet index overflow rejection - L1177; Prepare-rename coordinate overflow rejection - L1176; Outline document-symbol integer overflow rejection - L1175; DAP payload integer overflow rejection - L1174; Agents inspect integer overflow rejection - L1173; LSP navigation integer overflow rejection - L1172; LSP diagnostic coordinate overflow rejection - L1171; LSP payload integer overflow rejection - L1170; LSP diagnostic severity fallback - L1169; Web Playground local URL boundaries - L1168; Unified diff hunk range token boundaries - L1167; DAP frame content-length boundaries - L1166; Source-control divergence count token boundaries - L1165; Compiler-report zero-location rejection - L1164; Prepare-rename coordinate token boundaries - L1163; Outline document-symbol token boundaries - L1162; Completion deprecated-tag token boundaries - L1161; Agents inspect integer token boundaries - L1160; DAP payload integer token boundaries - L1159; LSP payload integer token boundaries - L1158; Mighty LSP response ID token boundaries - L1157; LSP JSON-RPC ID token boundaries - L1156; LSP apply-edit malformed request skipping - L1155; LSP execute-command apply-edit ownership - L1154; Mighty LSP deep-language capability parity - L1153; LSP document-symbol capability metadata - L1152; LSP rename prepare capability metadata - L1151; LSP signature-help capability metadata - L1150; LSP code-action capability metadata - L1149; LSP preferred code actions - L1148; LSP client capability parity - L1147; LSP completion capability handshake - L1146; LSP completion item defaults - L1145; LSP completion text edit ranges - L1144; LSP completion commit characters - L1143; LSP completion label details - L1142; LSP completion deprecated markers - L1141; LSP completion preselect - L1140; LSP completion documentation metadata - L1139; LSP completion kind metadata - L1138; LSP completion sort text - L1137; LSP completion provider detail - L1136; LSP completion display labels - L1135; LSP completion filter text - L1134; LSP completion insert text and snippets - L1133; snippet random and UUID variables - L1126; snippet Unix timestamp variables - L1125; GPU surface alpha stability - L1124; Surface-open and zoom toast ownership - L1123; Editor preflight toast ownership - L1122; Line-edit toast grouping - L1121; Command Palette unavailable toast grouping - L1120; Git blame target toast grouping - L1119; Agents live-inspect toast grouping - L1118; transient popup auto-dismiss regression guard - L1117; Keyboard Shortcuts capture dismissal - L1116; Save All duplicate-skip hardening - L1115; incidental prompt and toolbar dismissal cleanup - L1113/L1114. Prior: persisted recent files - L54; snippet mirror placeholders - L53; Explorer file operations + prompt-string staging pressure - L52; Windows packaging/runtime ABI hardening - L50/L51; multi-language support: config-driven highlighting + a generic, registry-configurable LSP bridge for non-Mighty languages - L35; verified live against rust-analyzer 1.95.0. Developer-workflow features - Run panel + inline git diff + live Settings panel - L33/L34; LIVE EDITING via a shim-side authoritative text model - the L28 workaround; command palette shim-side registry; L27.)_
...
- **IDE note:** code-action parsing now omits actions with top-level `disabled`
  metadata. The current quick-fix menu has no disabled-row affordance, so hiding
  them is clearer than exposing a selectable no-op; nested command arguments are
  still preserved for executable actions.
...
Hover cards, definition targets, signature help, completion menus, rename
inputs, snippet tab-stops, inline AI ghost text, code-action menus, quick-fix
lightbulbs, inline peek cards, and breadcrumb dropdowns are all computed from
the current buffer, current cursor, or current file folder. If they survive an
active-file transition, they can point at the previous tab while the status bar
and editor show a new file.
...
## L1116 - Capture Mode Still Needs Working Pointer Dismissal

Keyboard Shortcuts remap capture has different Escape semantics from the normal
overlay: Escape cancels capture and keeps the overlay open. That leaked into
pointer handling, so clicking the visible close affordance while capture was
active only cancelled capture and left the overlay visible.

- **IDE note:** capture-mode close-click and outside-click routes now hit-test
  the overlay, then call `mui_keys_dismiss` for close or miss paths. Escape
  still cancels capture without closing the overlay.
...
## L1120 - Palette Unavailable Messages Need Workflow Owners

Command Palette rows can surface preflight failures before dispatch, especially
when `MIGHTY_MTY` points at a missing compiler or a retained target has gone
stale. Those messages use row-specific wording such as `Run unavailable: ...`,
`Tests unavailable: ...`, `Debug unavailable: ...`, and `Browser run
unavailable: ...`, so prefix-only runtime grouping missed them.

- **IDE note:** palette unavailable messages now share the operation key for
  their owning workflow: Format, Run, Tests, Debug, Web, Mighty Agents, and New
  Project.
- **Language note:** no compiler gap surfaced. Palette preflight strings are
  first-class user feedback, so toast grouping tests should include them beside
  direct runtime command messages.

## L1121 - Line-Edit No-Ops Are Editor Feedback, Not Clipboard Feedback

The Command Palette can report line-edit preflight states before dispatch:
`Current line is already empty`, `Line is already at the top`, `Line is already
at the bottom`, and `No next line to join`. Those messages describe edit
commands, but without a dedicated operation key they could stack beside each
other or be awkwardly folded into clipboard feedback because copy/cut line
commands live nearby.

- **IDE note:** line-edit no-op messages now share a dedicated Edit operation
  key, and tests prove they replace each other while staying separate from
  clipboard and terminal feedback.
...
## L1123 - Surface-Open And Zoom Limit Feedback Need Owners Too

Command Palette rows also report idempotent surface and view states such as
`Search panel is already open`, `Source Control panel is already open`,
`Run and Debug panel is already open`, `Color theme picker is already open`,
and zoom limit messages. These are not errors, but they are visible feedback and
should replace stale messages in the same workflow instead of stacking as
unowned toasts.

- **IDE note:** already-open surface feedback now maps to its owning
  Navigation, Git, Debug, Theme, or Layout lane, and zoom limit messages share
  the layout lane. Toast tests cover the palette message shapes.
- **Language note:** no compiler gap surfaced. Idempotent command feedback needs
  the same ownership discipline as failures, because users trigger both from
  the same palette rows.

## L1124 - Surface Alpha Mode Should Stay Stable Across Resize

The windowed GPU path selected a surface alpha mode from adapter capabilities at
startup, but resize reconfigured the swapchain with `CompositeAlphaMode::Auto`.
That can silently change compositor behavior after a DPI or window resize,
which is the kind of platform-specific visual instability that makes native UI
feel less polished even when the first frame is correct.

- **IDE note:** the renderer now stores the chosen surface alpha mode and
  reuses it for every surface reconfiguration. Pure helper tests cover adapter
  alpha-mode selection, empty-capability fallback, and Vello-safe surface format
  fallback behavior without requiring a windowed GPU in CI.
...
## L1134 - LSP Completion Filter Text Is A Matching Key, Not Insert Text

After insert text support landed, semantic completions could still disappear
when the server supplied an abbreviated `filterText`. In LSP, `filterText`
controls whether an item matches the user's typed prefix; accepting the row
should still insert `textEdit.newText`, `insertText`, or the display label.

- **IDE note:** semantic candidates now carry optional filter text through the
  Rust-side completion engine. Prefix matching accepts either inserted text or
  filter text, but the scalar accept path still exposes only the inserted text
  to Mighty.
- **Language note:** no compiler gap surfaced. This is another case where the
  shim should keep protocol metadata rich until the last scalar boundary instead
  of reducing server data to one string too early.

## L1135 - LSP Completion Labels Are Display Text, Not Edit Text

Once the IDE started honoring `insertText` and `textEdit.newText`, the dropdown
could become noisy: a server might label a row `println!` but ask the client to
insert `println($1)`, or return a large snippet body as the edit text. Showing
the inserted body in the row makes semantic completion harder to scan.

- **IDE note:** completion candidates now keep inserted text and display text
  separately. LSP `label` is used for the dropdown when it differs from the edit
  text, while the scalar accept ABI still exposes only the inserted text.
...
## L1137 - LSP Completion Sort Text Is Part Of Ranking

After preserving label, filter, insert, snippet, and detail metadata, generic
completion still treated the server's response order as the only semantic row
ranking. Servers that use `sortText` to pin keywords, methods, snippets, or
deprioritized results could not express that ordering in Mighty IDE.

- **IDE note:** semantic candidates now preserve `CompletionItem.sortText`.
  When any semantic row supplies a sort key, the shim ranks semantic rows by
  provider sort key before appending buffer words; label-only compatibility
  paths keep their existing server order.
...
## L1144 - LSP Completion Text Edit Ranges Need Provider Width

Generic completion already preferred `textEdit.newText`, but accepting the row
still deleted only the identifier prefix before the cursor. Language servers can
return a wider `textEdit.range`, for example replacing `Console.Wr` with
...
## L1145 - LSP Completion Item Defaults Are List-Level Metadata

Modern completion lists can move repeated item metadata into
`CompletionList.itemDefaults`. A scraper that only sees each `CompletionItem`
object drops defaults such as shared commit characters or edit ranges, making
rows behave differently from servers that inline the same metadata per item.

- **IDE note:** semantic completion now keeps the surrounding `CompletionList`
  payload while scraping items. It applies `itemDefaults.commitCharacters` and
  safe `itemDefaults.editRange` replacement spans when individual items omit
  those fields, including insert/replace-shaped edit ranges. Per-item fields
  still win when present.
- **Language note:** no compiler gap surfaced. This reinforces that protocol
  structure should stay shim-owned until the final scalar facts are needed by
  Mighty.

## L1146 - LSP Completion Capabilities Must Match the Parser

The completion client had grown support for snippets, kind metadata,
documentation, deprecation tags, preselection, commit characters, insert/replace
edits, label details, and completion-list defaults, but its initialize request
still sent an empty capability object. Some language servers use those
capabilities to decide whether rich completion shapes are legal to return.

- **IDE note:** the completion handshake now advertises the item capabilities
  that the shim can consume. The scraper also honors `CompletionItem.textEditText`
  when the surrounding `CompletionList.itemDefaults.editRange` supplies the
  replacement span, so advertised list-default support has a corresponding edit
  text path.
- **Language note:** no compiler gap surfaced. Capability negotiation is another
  protocol boundary that belongs in the shim until Mighty has richer structured
  data support across the FFI boundary.

## L1147 - LSP Capability Declarations Need Cross-Client Parity

After the completion-only Mighty client started advertising rich completion
support, the registry-backed multi-language client still declared only snippet
completion, and the Mighty hover/definition client still declared no
text-document capabilities at all. That split can make different language
routes receive different server payload shapes even though the downstream
parsers can handle the richer responses.

- **IDE note:** the registry-backed LSP client now advertises the same completion
  item support as the completion-only path, plus markdown/plaintext hover content
  and definition link support. The Mighty hover/definition client also declares
  markdown/plaintext hover content and definition link support instead of an
  empty capability object.
- **Language note:** no compiler gap surfaced. This is protocol configuration
  drift in Rust-owned client code, not a Mighty language limitation.

## L1148 - LSP Preferred Code Actions Should Drive Initial Selection

Language servers can mark a quick fix with `CodeAction.isPreferred`, but the
IDE previously discarded that bit and selected the first actionable row. When a
server returns multiple fixes for the same diagnostic, Enter could apply a less
specific action even though the protocol already identified the best one.

- **IDE note:** code-action parsing now preserves top-level `isPreferred`, the
  quick-fix menu initially selects the first preferred actionable row, and
  preferred rows render with a distinct marker. Nested `isPreferred` fields in
  command arguments or metadata are ignored.
- **Language note:** no compiler gap surfaced. This is another protocol metadata
  case that stays in the shim until Mighty needs only the selected action.

## L1149 - LSP Code-Action Capabilities Should Match Menu Metadata

After preserving `CodeAction.isPreferred`, the registry-backed LSP client still
advertised `textDocument.codeAction` as an empty object. Servers can use client
capabilities to decide whether to return literal code actions, preferred markers,
or disabled actions.

- **IDE note:** the generic LSP initialize payload now declares code-action
  literal kinds, preferred-action support, and disabled-action support. It does
  not advertise `resolveSupport` or data flows because the IDE still expects
  actionable edits or commands in the initial response.
- **Language note:** no compiler gap surfaced. This is a protocol honesty issue
  in the Rust client boundary.

## L1150 - LSP Signature-Help Capabilities Should Match Parsed Metadata

The signature parser already preserves markdown/plaintext documentation and
parameter label offsets, but the generic LSP client advertised signature help as
an empty object. Servers can use client capabilities to decide whether those
richer signature shapes are worth returning.

- **IDE note:** the registry-backed initialize payload now declares
  `signatureInformation.documentationFormat` for markdown/plaintext and
  `parameterInformation.labelOffsetSupport`. It deliberately does not claim
  trigger-context support because the request path still sends only a position.
- **Language note:** no compiler gap surfaced. This is protocol configuration
  staying aligned with the Rust-owned parser/request boundary.

## L1151 - LSP Rename Capabilities Should Expose Prepare Support

The IDE already sends `textDocument/prepareRename`, handles explicit rejection,
and uses accepted ranges to seed the inline rename field. Both the generic LSP
client and the Mighty deep-language client still initialized with rename
capabilities that did not say prepare-rename was supported.

- **IDE note:** both initialize payloads now declare
  `textDocument.rename.prepareSupport`. They deliberately do not claim
  `honorsChangeAnnotations` because workspace-edit annotation handling is not
  part of the current scalar edit path.
- **Language note:** no compiler gap surfaced. This is another client
  capability declaration staying aligned with existing Rust-owned protocol
  behavior.

## L1152 - LSP Document-Symbol Capabilities Should Match Outline Parsing

The Outline parser already accepts hierarchical `DocumentSymbol[]` responses
with nested children and flat `SymbolInformation[]` responses with `kind`
values. The initialize payloads still declared document-symbol support as empty
or omitted it entirely, leaving servers to infer which outline shapes are safe
to return.

- **IDE note:** the generic and Mighty-specific initialize payloads now declare
  hierarchical document-symbol support plus the standard `SymbolKind` value set.
  They deliberately do not claim label or tag support because the current
  Outline surface only consumes names, kinds, ranges, and children.
- **Language note:** no compiler gap surfaced. This is protocol metadata being
  aligned with the Rust-owned Outline parser and scalar navigation state.

## L1153 - Mighty LSP Deep-Language Capabilities Need Parser Parity

The Mighty-specific deep-language client had been expanded to advertise
prepare-rename and document-symbol metadata, but it still omitted the
signature-help and code-action capabilities that its parser and UI already use.
That made the Mighty route less explicit than the generic route for the same
metadata shapes.

- **IDE note:** the Mighty initialize payload now declares signature
  markdown/plaintext docs, parameter label offsets, code-action literal kinds,
  preferred actions, and disabled actions alongside prepare-rename and
  document-symbol support. It still does not claim trigger-context,
  code-action resolve, or workspace-edit change-annotation flows.
- **Language note:** no compiler gap surfaced. This is protocol configuration
  parity between Rust-owned LSP clients, not a Mighty syntax or ABI limitation.

## L1154 - LSP Apply-Edit Responses Need Owned Edit Payloads

Some command-backed code actions ask the client to execute
`workspace/executeCommand`; a server can then send `workspace/applyEdit` before
the command response completes. The generic client answered those requests by
method and ID alone, so a malformed or metadata-only request could be reported
as applied even though no top-level edit was present.

- **IDE note:** the execute-command reader now acknowledges
  `workspace/applyEdit` only when the request owns `params.edit`. Nested
  metadata-only edits and missing-edit requests are ignored instead of getting a
  success response.
- **Language note:** no compiler gap surfaced. This is protocol envelope
  ownership hardening in the Rust LSP bridge.

## L1155 - LSP Apply-Edit Scanners Should Skip Malformed Requests

After requiring `workspace/applyEdit` to own `params.edit`, the same scanner
still treated a malformed apply-edit object with no usable request ID as a hard
miss. If a server or middleware emitted that object before a valid apply-edit
request in the same stream, the IDE could fail to acknowledge the later request.

- **IDE note:** the apply-edit request scanner now continues past malformed
  apply-edit objects with missing or invalid IDs and returns the first later
  request that owns both `params.edit` and a valid numeric/string ID.
- **Language note:** no compiler gap surfaced. This is another protocol
  streaming guard in Rust-owned LSP glue.

## L1156 - LSP JSON-RPC IDs Need Token Boundaries

The generic LSP response/apply-edit scanner parsed numeric request IDs by
reading an integer prefix. That meant malformed or fractional JSON-RPC IDs such
as `2.5` could be treated as `2`, causing the client to stop waiting for the
real response or acknowledge an apply-edit request with a different ID.

- **IDE note:** numeric JSON-RPC IDs now require a complete integer token
  followed by JSON value punctuation or whitespace. Fractional/exponent-like
  prefixes are skipped, letting later valid response or apply-edit IDs win.
- **Language note:** no compiler gap surfaced. This is byte-level protocol
  parsing hardening in the Rust-owned generic LSP client.

## L1157 - Mighty LSP Response IDs Need The Same Token Boundaries

The shared Mighty-specific response scanner for hover, definition, completion,
signature help, rename, code actions, and outline reused a top-level unsigned
integer helper that also accepted numeric prefixes. A malformed response with
`id:2.5` could therefore satisfy the `id:2` wait or isolate the wrong payload
before the real response arrived.

- **IDE note:** the shared Mighty LSP numeric-field parser now requires a
  complete JSON integer token boundary before `has_response_id` or
  `isolate_response_id` accepts the response. Feature-specific tests in nav,
  completion, and language pin the behavior.
- **Language note:** no compiler gap surfaced. This is shared protocol parsing
  hardening across the Mighty-specific LSP routes.

## L1158 - LSP Payload Integers Need Token Boundaries Too

After hardening JSON-RPC envelope IDs, LSP payload fields still reused unsigned
integer helpers that accepted numeric prefixes. Fractional values in
signature-help active indexes, parameter offset labels, workspace-edit ranges,
or generic diagnostic ranges could be truncated into plausible but wrong
positions.

- **IDE note:** Mighty language-action payload parsing and generic diagnostic
  parsing now require complete unsigned integer tokens. Malformed positions are
  ignored or fall back instead of being truncated into edits, active-parameter
  indexes, or diagnostics.
- **Language note:** no compiler gap surfaced. This is protocol payload parsing
  hardening in Rust-owned LSP glue.

## L1159 - DAP Payload Integers Need Token Boundaries

The debugger adapter parser reused a top-level integer helper for response
`request_seq`, stopped-thread IDs, thread IDs, stack-frame IDs/lines, and exit
codes. That helper accepted integer prefixes, so malformed payloads such as
`threadId:1.5` or `line:8.5` could be truncated into plausible debugger state.

- **IDE note:** DAP numeric fields now require complete integer token
  boundaries. Fractional debugger payloads are ignored or fall back instead of
  driving thread lookup, stack frame selection, stopped-line highlighting, or
  exit-code reporting with truncated numbers.
- **Language note:** no compiler gap surfaced. This is debugger protocol
  parsing hardening in the Rust-owned DAP client.

## L1160 - Agents Inspect Integers Need Token Boundaries

The live Agents panel parses `mty inspect --json` snapshots with a small
top-level scanner so it can avoid a wider runtime dependency. Its unsigned
integer helper also accepted prefixes, so malformed runtime snapshots such as
`"agent_id":7.5` or `"mailbox_depth":2.5` could be truncated into plausible
Agents rows.

- **IDE note:** live-inspect `worker_count`, `agent_id`, `mailbox_depth`, and
  `mailbox_high_water` fields now require complete integer tokens. Malformed
  worker counts fall back to zero, malformed agent IDs skip the row, and
  malformed counters fall back instead of displaying truncated queue state.
- **Language note:** no compiler gap surfaced. This is runtime inspection JSON
  hardening in the Rust-owned Agents panel.

## L1161 - Completion Deprecated Tags Need Token Boundaries

Semantic completion already used complete integer parsing for `kind`,
`insertTextFormat`, and edit ranges, but the `CompletionItem.tags` array scanner
looked for numeric prefixes when checking for tag `1` (`Deprecated`). Malformed
tag values such as `1.5` or `1e0` could therefore mark a fresh completion row as
deprecated.

- **IDE note:** LSP completion tag entries now require a complete integer token
  before they can set the deprecated marker. Valid `tags:[1]` still works, while
  fractional or exponent-style prefixes are ignored.
- **Language note:** no compiler gap surfaced. This is completion metadata
  parsing hardening in the Rust-owned LSP autocomplete bridge.

## L1162 - Outline Document-Symbol Integers Need Token Boundaries

The Outline panel's document-symbol parser used shared integer helpers for LSP
error codes, `SymbolKind`, and range start lines. Those helpers accepted numeric
prefixes, so malformed payloads such as `code:-32601.5`, `kind:23.5`, or
`line:2.5` could trigger scanner fallback, classify a symbol, or jump to a
truncated line.

- **IDE note:** outline numeric fields now require complete integer tokens.
  Malformed method-not-found error codes no longer activate scanner fallback,
  malformed symbol kinds fall back to the default kind, and malformed range
  lines skip that LSP row instead of jumping to a truncated line.
- **Language note:** no compiler gap surfaced. This is document-symbol parsing
  hardening in the Rust-owned Outline/LSP bridge.

## L1163 - Prepare-Rename Coordinates Need Token Boundaries

The symbol-rename path parses `prepareRename` result ranges before opening the
inline rename prompt. Its coordinate helper still accepted unsigned integer
prefixes, so malformed ranges such as `"line":4.5` or `"character":8.5` could
position the prompt on a truncated coordinate.

- **IDE note:** prepare-rename `start.line` and `start.character` now require
  complete integer tokens. Malformed range coordinates are rejected, leaving the
  rename command on the cursor-derived symbol fallback instead of moving to a
  truncated server coordinate.
- **Language note:** no compiler gap surfaced. This is rename protocol parsing
  hardening in Rust-owned code-intelligence glue.

## L1164 - Compiler Report Locations Are 1-Based

The Problems parser and Run panel both read human-readable Mighty compiler
locations from `[...]` report lines. Those locations are documented and emitted
as 1-based `line:column` pairs, but malformed `0:0` records were previously
accepted and clamped into a real top-left editor location.

- **IDE note:** `mty check` diagnostics and streamed Run output now reject zero
  line or column values before attaching a diagnostic location or clickable run
  output target. Header-only diagnostics still fall back to 0:0 as an explicit
  "no precise location" state, but malformed location records no longer create
  clickable jumps.
- **Language note:** no compiler gap surfaced. This is report-parser hygiene in
  Rust-owned diagnostics and Run output glue.

## L1165 - Git Divergence Counts Are Unsigned Decimal Tokens

Source Control reads `ahead` / `behind` counts from `git status --porcelain=v1
-b` branch headers. Those counts are non-negative decimal commit counts, but the
parser previously accepted signed integers and treated other malformed tokens as
zero without an explicit token boundary.

- **IDE note:** branch divergence parsing now accepts only unsigned decimal
  tokens before updating Source Control sync totals. Signed, fractional, or
  exponent-style counts are ignored instead of surfacing bogus ahead/behind
  state.
- **Language note:** no compiler gap surfaced. This is source-control parser
  hygiene in Rust-owned git integration code.

## L1166 - DAP Frame Lengths Are Positive Decimal Tokens

The Debug Adapter Protocol reader consumes `Content-Length` framed JSON from
`mty dap`. A malformed length header previously fell back to zero and produced
an empty frame, which hid protocol corruption and risked losing framing
alignment.

- **IDE note:** DAP message reads now require a present, positive, unsigned
  decimal `Content-Length` before reading a body. Zero, signed, fractional,
  exponent-style, or missing lengths become invalid-data read errors instead of
  empty debugger messages.
- **Language note:** no compiler gap surfaced. This is debugger transport
  hygiene in Rust-owned DAP integration code.

## L1167 - Unified Diff Hunk Ranges Are Complete Decimal Tokens

Inline Source Control diffs parse unified hunk headers before assigning old/new
line numbers and before reconstructing one-hunk patches for stage/unstage
actions. The parser previously read only the range start, so malformed optional
count fields could still create display rows and patchable hunk actions.

- **IDE note:** hunk headers now validate both range starts and optional counts
  as complete unsigned decimal tokens. Negative starts, bad count fields, and
  extra range fields are skipped before a hunk can assign line numbers or become
  reconstructable for patch actions.
- **Language note:** no compiler gap surfaced. This is source-control diff
  parser hygiene in Rust-owned git integration code.

## L1168 - Web Playground Auto-Open URLs Stay Local

Web Playground scrapes served URLs from build and server output so the IDE can
open the browser once the local server is ready. The scraper previously latched
the first `http` token, which meant an external documentation link or malformed
URL in output could be treated like the served app.

- **IDE note:** URL scraping now accepts only loopback `http` / `https` URLs
  with a positive decimal port. `localhost`, `127.*`, and `[::1]` are accepted;
  external hosts, missing ports, zero ports, signed ports, and exponent-style
  ports are ignored before the browser auto-open latch is set.
- **Language note:** no compiler gap surfaced. This is Web Playground output
  parsing hygiene in Rust-owned web-run integration code.

## L1169 - LSP Diagnostic Severities Fail Soft

LSP diagnostics use numeric severity values where `1` is error and `2..=4`
are progressively less severe. The IDE only has error and warning display
states, so the bridge already folded information and hint severities into
warnings. Malformed or missing severity fields, however, previously fell
through the parser's default error path.

- **IDE note:** diagnostic severity parsing now accepts only complete unsigned
  numeric severity tokens. `1` remains an error, `2..=4` remain warnings, and
  malformed, missing, or unsupported severities fall back to warning display so
  questionable server payloads stay visible without being overstated.
- **Language note:** no compiler gap surfaced. This is LSP payload parser
  hygiene in Rust-owned language-server integration code.

## L1170 - LSP Payload Integers Reject Overflow

The Mighty-specific language feature parser reads signature help,
parameter-offset labels, and workspace edits from LSP JSON. It already rejected
fractional numeric prefixes, but its shared unsigned scanner saturated values
larger than `u32::MAX` to `u32::MAX`, letting malformed payloads become extreme
coordinates that downstream editor code could clamp into real edits or popup
state.

- **IDE note:** the shared language-feature unsigned scanner now uses checked
  arithmetic. Overflowing `activeSignature`, `activeParameter`, parameter-label
  offsets, and workspace-edit range coordinates are treated like malformed
  fields: optional indexes fall back to zero and required edit coordinates skip
  that edit instead of applying saturated positions.
- **Language note:** no compiler gap surfaced. This is LSP payload parser
  hygiene in Rust-owned language-server integration code.

## L1171 - LSP Diagnostic Coordinates Reject Overflow

LSP diagnostics use unsigned line and character positions, while Mighty IDE's
shared Problems model stores display coordinates as signed integers. The
diagnostic parser already rejected fractional numeric prefixes, but unsigned
overflow could saturate and values above `i32::MAX` could wrap negative during
the final conversion into the editor diagnostic type.

- **IDE note:** publish-diagnostics range parsing now uses checked unsigned
  arithmetic and checked conversion into signed editor coordinates. Diagnostics
  with overflowing line/character tokens or coordinates outside the signed
  display range are skipped instead of creating bogus Problems rows or negative
  underline positions.
- **Language note:** no compiler gap surfaced. This is LSP payload parser
  hygiene in Rust-owned diagnostic integration code.

## L1172 - LSP Navigation Integers Reject Overflow

Hover and Go to Definition share a small navigation JSON scanner for response
IDs and definition target coordinates. It already required complete integer
tokens, but overflowing unsigned tokens saturated to `u32::MAX`, which could
turn malformed responses into real response matches or extreme definition
targets.

- **IDE note:** the navigation parser now uses checked unsigned arithmetic for
  JSON-RPC response IDs and `Location` / `LocationLink` start positions.
  Overflowing IDs are ignored during response isolation, and overflowing
  definition target coordinates make that target invalid instead of jumping to
  a saturated line or column.
- **Language note:** no compiler gap surfaced. This is LSP payload parser
  hygiene in Rust-owned navigation integration code.

## L1173 - Agents Inspect Integers Reject Overflow

The live Agents panel parses `mty inspect --json` snapshots with a small
top-level scanner. It already rejected fractional numeric prefixes, but
overflowing `u64` values saturated to `u64::MAX`, which could show impossible
worker counts, agent IDs, or mailbox depths as if they were real telemetry.

- **IDE note:** live-inspect `worker_count`, `agent_id`, `mailbox_depth`, and
  `mailbox_high_water` fields now use checked unsigned arithmetic. Overflowing
  worker counts and optional counters fall back to zero, while overflowing
  required agent IDs skip the malformed row.
- **Language note:** no compiler gap surfaced. This is runtime-inspection JSON
  parser hygiene in Rust-owned Agents integration code.

## L1174 - DAP Payload Integers Reject Overflow

The debugger parser shares a small unsigned scanner for DAP envelope sequence
IDs, stopped-thread IDs, thread rows, stack frame rows, and exit codes. It
already rejected fractional numeric prefixes, but overflowing values saturated
to `i64::MAX`, which could surface impossible debug state as if it came from a
trusted adapter.

- **IDE note:** DAP numeric fields now use checked integer arithmetic.
  Overflowing response sequence IDs and stopped-thread IDs become absent,
  overflowing exit codes fall back to zero, and overflowing thread IDs,
  stack-frame IDs, or stack-frame line numbers skip the malformed row.
- **Language note:** no compiler gap surfaced. This is debugger payload parser
  hygiene in Rust-owned DAP integration code.

## L1175 - Outline Document-Symbol Integers Reject Overflow

The LSP outline parser shares small integer scanners for error codes,
`SymbolKind` values, and document-symbol range start lines. It already rejected
fractional numeric prefixes, but overflowing values saturated to maximum
integers, which could misclassify symbols, keep bogus outline rows, or treat a
malformed error code as a scanner-fallback signal.

- **IDE note:** outline document-symbol integer fields now use checked
  arithmetic. Overflowing error codes are treated as malformed LSP errors
  without falling back to the scanner, overflowing `SymbolKind` values fall back
  to the generic constant presentation, and overflowing range start lines skip
  the malformed symbol row.
- **Language note:** no compiler gap surfaced. This is LSP outline payload
  parser hygiene in Rust-owned document-symbol integration code.

## L1176 - Prepare-Rename Coordinates Reject Overflow

Generic language-server `prepareRename` responses use a small JSON scanner to
read the accepted range's `start.line` and `start.character`. It already
rejected fractional numeric prefixes, but overflowing unsigned coordinates
saturated to `u32::MAX`, which could turn malformed server replies into extreme
rename-symbol lookup positions.

- **IDE note:** prepare-rename range coordinates now use checked unsigned
  arithmetic. Overflowing `start.line` or `start.character` makes the server
  range unusable, preserving the cursor-derived fallback symbol instead of
  looking up a saturated location.
- **Language note:** no compiler gap surfaced. This is LSP rename payload
  parser hygiene in Rust-owned ABI integration code.

## L1177 - Snippet Indexes Reject Overflow

Imported snippet bodies and transform replacements use decimal indexes for
tab-stops and regex captures. Those scanners previously saturated overflowing
values, which could create impossible tab-stop IDs or treat a malformed capture
reference as a valid request for `usize::MAX`.

- **IDE note:** snippet tab-stop and transform capture indexes now use checked
  parsing. Overflowing bare or braced tab-stop indexes are preserved as literal
  text instead of opening a saturated snippet session, overflowing unbraced
  capture replacements resolve to an empty capture, and overflowing braced
  capture formats stay literal.
- **Language note:** no compiler gap surfaced. This is imported-snippet parser
  hygiene in Rust-owned snippet expansion code.

## L1178 - Run Output Location Labels Need Token Boundaries

Run output rows use compact labels for bracketed compiler locations like
`[path:line:column]` when a row has no clickable target. The helper used
`parse::<u32>()`, which accepts `+1`, so malformed streamed diagnostics could
be presented as clickable-looking source labels.

- **IDE note:** Run output row labels now accept only positive decimal
  line/column tokens. Signed-looking, zero, fractional, or overflowing
  coordinates fall back to the ordinary compact row text instead of a source
  location label.
- **Language note:** no compiler gap surfaced. This is Run-panel output parser
  hygiene in Rust-owned developer-workflow glue.

## L1179 - Completion Numeric Fields Reject Plus-Prefixed Tokens

Generic LSP completion payloads use numeric fields for completion kind,
snippet insert-text format, and text-edit ranges. The parser already rejected
fractional/exponent forms through integer parsing, but Rust's integer parser
accepted `+N`, which is not a JSON number shape and could let malformed
completion items claim function styling, snippet flattening, or replacement
ranges.

- **IDE note:** completion numeric fields now validate token shape before
  parsing. Plus-prefixed `kind`, `insertTextFormat`, and edit-range coordinates
  are ignored, while ordinary signed/unsigned integer tokens keep their
  previous behavior.
- **Language note:** no compiler gap surfaced. This is LSP completion payload
  parser hygiene in Rust-owned autocomplete integration code.
...
## L1181 - Terminal CSI/OSC Numeric Parameters Need Token Boundaries

The integrated terminal parses CSI and OSC numeric parameters for cursor
movement, erasing, SGR styling, and xterm palette queries/updates. Direct Rust
integer parsing accepts `+N`, so malformed terminal escape sequences could move
the cursor, style text, or target palette slots as if those parameters were
ordinary decimal tokens.

- **IDE note:** terminal CSI counts, cursor positions, SGR semicolon/colon color
  parameters, and OSC palette indices now validate digit-only decimal tokens
  before parsing. Plus-prefixed numeric-looking values are ignored or fall back
  to each sequence's empty/default behavior instead of being accepted as valid
  controls.
- **Language note:** no compiler gap surfaced. This is terminal escape parser
  hygiene in Rust-owned PTY integration code.

## L1182 - Persisted Shortcut Overrides Need Numeric Token Boundaries

Keyboard shortcut remaps load from `keybindings.toml` as `cmd_<id>` entries
with `mods:codepoint` values. The parser skipped malformed lines, but direct
Rust integer parsing still accepted plus-prefixed values, so a hand-edited or
corrupt config file could silently apply a remap from `cmd_+N` or `+4:+107`.

- **IDE note:** persisted shortcut override command IDs and `mods:codepoint`
  values now validate signed/unsigned decimal token shape before parsing.
  Plus-prefixed numeric-looking keys or values are ignored instead of loading
  as active shortcut remaps.
- **Language note:** no compiler gap surfaced. This is keyboard-shortcut config
  parser hygiene in Rust-owned UI state code.

## L1183 - Persisted Numeric Preferences Need Token Boundaries

Editor settings and saved zoom load numeric values from user-editable config
files at startup. Direct Rust numeric parsing accepts plus-prefixed tokens and
float exponent forms, so a hand-edited or corrupt config could silently apply
malformed `font_size`, `tab_width`, or zoom values as if they were clean
decimal preferences.

- **IDE note:** persisted `font_size`, `tab_width`, and zoom values now use
  shared decimal-token validators before parsing. Plus-prefixed, suffixed,
  exponent-form, or dot-only numeric-looking values are ignored, while ordinary
  decimal values keep the existing clamp behavior.
- **Language note:** no compiler gap surfaced. This is persisted-preference
  parser hygiene in Rust-owned startup config code.

## L1184 - Quick Open and Testing Counts Need Token Boundaries

Quick Open's `:line` mode and the Testing panel's parsed `mty test` summary
counts both consume numeric text that can be typed by a user or emitted by an
external tool. Direct Rust integer parsing accepts plus-prefixed values, and a
wide go-to-line value can exceed the signed editor ABI range.

- **IDE note:** Quick Open go-to-line input now accepts only positive unsigned
  decimal tokens that fit the signed editor line ABI, and Testing summary
  counts now accept only complete unsigned decimal tokens. Plus-prefixed,
  signed-looking, suffixed, fractional, or overflowing values stay invalid
  instead of driving navigation previews or final test totals.
- **Language note:** no compiler gap surfaced. This is command-surface and
  test-output parser hygiene in Rust-owned UI integration code.

## L1185 - Web Playground Port Overrides Need Token Boundaries

Web Playground already validates scraped server URLs before auto-opening a
browser, including a loopback host and a positive decimal port. The manual
`MIGHTY_WEB_PORT` override still used direct Rust `u16` parsing, which accepts
plus-prefixed values, so malformed automation input could drive the server bind
port even though equivalent scraped URL tokens were rejected.

- **IDE note:** `MIGHTY_WEB_PORT` now uses the same complete unsigned decimal
  token validation as Web Playground URL ports and still requires a positive
  nonzero port. Plus-prefixed, zero, overflowing, or partial values fall back to
  the default port instead of launching with malformed input.
- **Language note:** no compiler gap surfaced. This is Web Playground
  environment-parser hygiene in Rust-owned runtime integration code.

## L1186 - Markdown Ordered-List Overflow Should Preserve Structure

Markdown preview recognizes ordered list markers by shape, but the stored item
number is a `u64`. A marker with more digits than `u64` can hold is still
list-shaped input, and treating it as a paragraph breaks the surrounding live
preview structure.

- **IDE note:** ordered-list markers with oversized decimal text now remain
  ordered list items. The parsed number is stored as `None`, so the renderer
  uses fallback numbering while preserving the item content and contiguous list.
- **Language note:** no compiler gap surfaced. This is Markdown parser
  resilience in Rust-owned preview code.

## L1187 - Markdown Ordered-List Fallback Numbering Should Be One-Based

After oversized ordered-list markers are preserved as list items, the renderer
still needs to display a plausible marker when the parsed number is unavailable.
Rendering `0.` for an overflowed first item makes the preview look broken even
though the parser kept the list structure.

- **IDE note:** Markdown preview now renders ordered list items without a parsed
  number using the item's one-based display position. Explicit numbers such as
  `7.` still render as typed, while overflowed markers fall back to `1.`, `2.`,
  and so on.
- **Language note:** no compiler gap surfaced. This is Markdown renderer polish
  in Rust-owned preview code.

## L1188 - Git Blame Author-Time Needs Token Boundaries

The blame gutter consumes `git blame --porcelain` metadata from an external
tool. Its `author-time` header is expected to be an unsigned Unix timestamp,
but direct integer parsing accepts plus-prefixed values and can make malformed
repeat metadata look valid.

- **IDE note:** blame `author-time` now accepts only complete unsigned decimal
  tokens that fit `i64`. Plus-prefixed, signed, suffixed, or overflowing values
  are ignored, so malformed repeat metadata cannot replace the cached commit
  date already shown for that sha.
- **Language note:** no compiler gap surfaced. This is VCS-output parser hygiene
  in Rust-owned source-control integration code.

## L1189 - Compiler Diagnostic Locations Need Token Boundaries

Mighty compiler diagnostics include a location line with `path:line:column`.
Those line/column fields come from external compiler output and drive editor
underlines and navigation. Direct unsigned integer parsing accepts
plus-prefixed numbers, making malformed reports look valid.

- **IDE note:** `mty check` location parsing now accepts only complete unsigned
  decimal line/column tokens that fit `u32` and remain nonzero. Plus-prefixed,
  signed-looking, suffixed, zero, or overflowing values leave the diagnostic at
  the safe default location instead of driving bogus underline or jump
  positions.
- **Language note:** no compiler gap surfaced. This is compiler-output parser
  hygiene in Rust-owned diagnostics integration code.

## L1190 - Run Output Clickable Locations Need Token Boundaries

The Run panel streams compiler and process output, then turns ariadne-style
`[path:line:column]` rows into clickable editor jumps. That parser is separate
from the Problems diagnostics parser, so it needs the same boundary discipline
for streamed external output.

- **IDE note:** Run output clickable locations now accept only complete unsigned
  decimal line/column tokens that fit `u32` and remain nonzero. Plus-prefixed,
  signed-looking, suffixed, zero, or overflowing values stay plain output rows
  instead of becoming bogus source jumps.
- **Language note:** no compiler gap surfaced. This is streamed process-output
  parser hygiene in Rust-owned Run panel integration code.

## L1191 - Git Blame Timezones Need Digit Boundaries

The blame gutter applies `author-tz` offsets from `git blame --porcelain` so
dates match the author's local day. The parser checked the leading sign and the
overall width, but parsed the hour/minute slices directly, which let embedded
plus signs look like valid numeric slices.

- **IDE note:** blame `author-tz` now requires exactly four ASCII digits after
  the leading `+` or `-` and still enforces `HH <= 23` and `MM <= 59`.
  Malformed offsets such as `++000`, `-+000`, or `+00+0` are ignored so repeat
  metadata cannot shift cached blame dates.
- **Language note:** no compiler gap surfaced. This is VCS-output parser
  hygiene in Rust-owned source-control integration code.

## L1192 - DAP Frames Need a Pre-Allocation Size Cap

The debugger reads `Content-Length` framed JSON from an external debug adapter.
The parser already rejected malformed or zero lengths, but a syntactically valid
huge length could still ask the IDE to reserve an enormous body buffer before
any JSON bytes arrived.

- **IDE note:** inbound DAP frames are now capped at 16 MiB before allocation.
  Larger `Content-Length` headers fail as invalid data, just like malformed
  signed, fractional, zero, or exponent-style lengths.
- **Language note:** no compiler gap surfaced. This is debugger protocol
  resource-bound hygiene in Rust-owned DAP integration code.

## L1193 - Generic LSP Output Caps Should Fail Closed

The generic LSP bridge reads stdout from external language servers until the
requested response or diagnostics arrive. It already had byte caps, but the
capped path still returned the partial buffer, leaving room for partial or
truncated data to be parsed after the cap was crossed.

- **IDE note:** generic LSP request and diagnostics readers now discard the
  buffered stream when response or diagnostic output crosses its cap. Oversized
  server output fails closed instead of feeding partial response or diagnostic
  data into the UI.
- **Language note:** no compiler gap surfaced. This is external language-server
  resource-bound hygiene in Rust-owned multi-language LSP integration code.

## L1194 - Mighty LSP Output Caps Should Also Fail Closed

The specialized `mty lsp` clients for completion, hover/definition, and
language actions had the same resource-bound shape as the generic bridge: they
stopped reading after a response cap, but still returned the buffered partial
stream to the parser.

- **IDE note:** Mighty LSP response readers now share a single capped append
  helper. Crossing the cap clears the buffer before returning, so oversized
  `mty lsp` output cannot feed truncated completion, hover, definition,
  signature, rename, code-action, or outline responses into editor surfaces.
- **Language note:** no compiler gap surfaced. This is first-party LSP
  resource-bound hygiene in Rust-owned Mighty language-intelligence clients.

## L1195 - Clipboard Reads Need a Size Budget

Editor paste, terminal paste, and imported snippet `$CLIPBOARD` expansion all
read text through the platform clipboard command. The previous implementation
used `Command::output()`, so a broken or hostile clipboard provider could make
the IDE allocate the full stdout payload before any paste preflight rejected it.

- **IDE note:** clipboard reads now stream stdout through a 4 MiB cap and apply
  the same cap to the test/env clipboard source. Oversized clipboard text fails
  with `clipboard text too large`, leaving editor and terminal buffers unchanged.
- **Language note:** no compiler gap surfaced. This is host-integration
  resource-bound hygiene in Rust-owned clipboard plumbing.

## L1196 - Source Control Git Output Needs a Size Budget

Source Control push, pull, fetch, and branch actions only need a short toast
summary from git, but the previous runner captured full stdout/stderr with
`Command::output()`. A noisy remote hook or failing command could allocate
unbounded output before the summary was collapsed.

- **IDE note:** Source Control git actions now stream stdout and stderr through
  separate 2 MiB caps. If either stream crosses its budget the child process is
  killed and the action reports `git output too large` instead of allocating or
  summarizing partial output.
- **Language note:** no compiler gap surfaced. This is VCS process-output
  resource-bound hygiene in Rust-owned Source Control integration code.

## L1197 - Diagnostics Output Needs a Size Budget

Live diagnostics invoke `mty check` and parse its combined stdout/stderr into
Problem rows. The parser only needs bounded compiler reports, but the previous
runner used `Command::output()`, so a broken checker could allocate unbounded
stdout or stderr before diagnostics parsing began.

- **IDE note:** Mighty diagnostics now stream stdout and stderr through separate
  4 MiB caps. If either stream crosses its budget the child process is killed
  and the refresh reports `Diagnostics failed: mty check output too large`
  instead of allocating or parsing partial output.
- **Language note:** no compiler gap surfaced. This is compiler-process
  resource-bound hygiene in Rust-owned diagnostics integration code.

## L1198 - Agents Inspect Output Needs a Size Budget

The Agents panel can attach to a running Mighty runtime by invoking
`mty inspect --json` and parsing a live snapshot. That snapshot is useful only
when it is bounded, but the previous inspect path captured full stdout/stderr
with `Command::output()`, so a noisy runtime or transport failure could allocate
unbounded JSON or error text before parsing began.

- **IDE note:** Agents live inspect now streams stdout and stderr through
  separate 4 MiB caps. If either stream crosses its budget the child process is
  killed and the panel reports live inspect as unavailable instead of allocating
  or parsing partial snapshot output.
- **Language note:** no compiler gap surfaced. This is runtime-inspection
  resource-bound hygiene in Rust-owned Agents integration code.

## L1199 - Inline Diff Output Needs a Size Budget

The inline diff view shells out to `git diff`, then parses unified hunks for the
Source Control diff surface. That parser only needs a bounded patch, but the
previous runner captured the entire stdout payload with `Command::output()`.
Large generated files or noisy diff drivers could allocate unbounded patch text
before hunk parsing began.

- **IDE note:** inline Source Control diff output now streams stdout and stderr
  through separate 8 MiB caps. If either stream crosses its budget the child
  process is killed and the diff request fails closed instead of allocating or
  parsing partial patch output.
- **Language note:** no compiler gap surfaced. This is VCS diff-output
  resource-bound hygiene in Rust-owned inline diff integration code.

## L1200 - Diff Hunk Apply Output Needs the Same Budget

Single-hunk stage and unstage actions reconstruct a patch and feed it to
`git apply --cached`. The path already avoided unbounded patch input because the
patch comes from the parsed inline diff, but failed apply commands still used
`wait_with_output()` and could capture unbounded stdout/stderr before reporting
the Git error.

- **IDE note:** diff hunk stage/unstage now reads `git apply` stdout and stderr
  through the same 8 MiB per-stream budget as inline diff loading. Normal
  non-zero Git output is still preserved for feedback, while oversized output
  reports `git apply output too large`.
- **Language note:** no compiler gap surfaced. This is VCS apply-output
  resource-bound hygiene in Rust-owned inline diff integration code.

## L1201 - Formatter Output Needs a Size Budget

Format Document saves the active Mighty file, runs `mty fmt`, and reloads the
formatted disk bytes. The command already protects non-`.mty` files and dirty
duplicates, but formatter failures still used `Command::output()` and could
capture unbounded stdout/stderr before reporting the failure.

- **IDE note:** `mty fmt` now streams stdout and stderr through separate 4 MiB
  caps. Normal non-zero formatter stderr is still preserved for feedback, while
  oversized output reports `formatter output too large`.
- **Language note:** no compiler gap surfaced. This is formatter-process
  resource-bound hygiene in Rust-owned Format Document integration code.

## L1202 - New Project Output Needs a Size Budget

The New Project flow invokes `mty new` in the selected parent directory and
surfaces the final scaffold error line when creation fails. The command already
preflights invalid names, existing non-empty folders, and missing parents, but
the compiler process still used `Command::output()` and could allocate
unbounded stdout/stderr before the toast was produced.

- **IDE note:** `mty new` now streams stdout and stderr through separate 4 MiB
  caps. Normal scaffold failure stderr is still preserved for feedback, while
  oversized output reports `New project failed: mty new output too large`.
- **Language note:** no compiler gap surfaced. This is project-scaffold
  resource-bound hygiene in Rust-owned New Project integration code.

## L1203 - Web Playground Build Output Needs a Size Budget

The Web Playground fallback path runs `mty build --target wasm32-web` for bare
Mighty files before starting a static server. The build output is useful in the
panel, but the previous synchronous runner used `Command::output()` and could
allocate unbounded compiler stdout/stderr before the panel could show failure.

- **IDE note:** Web Playground fallback builds now stream stdout and stderr
  through separate 4 MiB caps. Normal build output is still fed into the panel,
  while oversized output reports `mty build output too large` and fails the run.
- **Language note:** no compiler gap surfaced. This is web-build process
  resource-bound hygiene in Rust-owned Web Playground integration code.

## L1204 - Source Control Read Output Needs a Size Budget

Source Control action commands already capped push, pull, fetch, and branch
switch output, but read-side Git calls still used `Command::output()` for repo
discovery, status refresh, branch listing, and single-file diff reads. Those
paths can run while opening or refreshing a repository, so they need the same
resource budget as explicit actions.

- **IDE note:** Source Control read-side Git commands now stream stdout and
  stderr through the shared 2 MiB per-stream Git cap. Normal status, branch,
  and diff output still parses as before; oversized output is discarded and the
  read path falls back to empty/no-repo state instead of allocating unbounded
  buffers.
- **Language note:** no compiler gap surfaced. This is VCS read-output
  resource-bound hygiene in Rust-owned Source Control integration code.

## L1205 - Git Blame Output Needs a Size Budget

The blame gutter consumes `git blame --porcelain` output before parsing and
caching per-line author/date metadata. Large files or unusual Git behavior could
previously make the blame toggle allocate all stdout/stderr via
`Command::output()` before the parser had a chance to reject anything.

- **IDE note:** Git blame now streams stdout and stderr through separate 8 MiB
  caps. Normal porcelain output still parses into cached blame rows; oversized
  output is discarded and the blame load follows the existing empty/no-blame
  path instead of allocating unbounded buffers.
- **Language note:** no compiler gap surfaced. This is VCS blame-output
  resource-bound hygiene in Rust-owned Git blame integration code.

## L1206 - Native Dialog Output Needs a Size Budget

The Windows native file, folder, save, new-file, and new-project dialogs are
launched through small PowerShell snippets that print the selected path on
stdout. The expected payload is a single filesystem path, but the helpers still
used `Command::output()` and could allocate unbounded stdout/stderr if the host
process behaved unexpectedly.

- **IDE note:** native dialog helpers now stream stdout and stderr through
  separate 64 KiB caps before accepting the selected path. Normal picker output
  is unchanged; oversized dialog output is treated as unavailable so the caller
  uses the existing typed-path fallback or no-op cancellation path.
- **Language note:** no compiler gap surfaced. This is platform-dialog
  resource-bound hygiene in Rust-owned file/workspace picker integration code.

## L1207 - Oversized File Opens Need A Metadata Gate

File-backed tabs previously called `std::fs::read` as the first step of opening
a path. Binary files were protected after the read by switching to a read-only
preview, but very large text or binary files could still be allocated in full
before the editor knew it should refuse normal editing.

- **IDE note:** tab opening now checks file metadata before reading. Files above
  16 MiB open as lightweight read-only large-file previews, keeping the path
  inspectable while avoiding full-buffer allocation in the text editor.
- **Language note:** no compiler gap surfaced. This is editor file-ingest
  resource-bound hygiene in Rust-owned tab loading code.

## L1208 - Project Search Needs A Per-File Read Budget

Project-wide search capped directory count and match count, but each candidate
file still used `std::fs::read` before binary detection or text matching. A
large generated log, bundle, or sparse artifact inside the workspace could
therefore allocate in full during a routine search refresh.

- **IDE note:** project search now checks candidate file metadata before
  reading. Files above 4 MiB are skipped during search, and replace-all treats
  a previously matched file that grows past the budget as changed/stale instead
  of reading and rewriting it.
- **Language note:** no compiler gap surfaced. This is workspace-search
  resource-bound hygiene in Rust-owned search indexing code.

## L1209 - Peek Preview Reads Need A File Budget

Cross-file Peek Definition only needs a small source window around the target
line, but the disk-backed preview path still read the entire target file before
extracting that window. Large generated sources or stale navigation targets
could therefore allocate on a quick code-reading gesture.

- **IDE note:** cross-file peek now checks file metadata before reading and
  returns the existing empty best-effort preview for targets above 4 MiB.
  In-memory current-buffer peek remains unchanged.
- **Language note:** no compiler gap surfaced. This is navigation preview
  resource-bound hygiene in Rust-owned peek rendering code.

## L1210 - Reload And Revert Need The Open-File Budget

Opening a file had a 16 MiB metadata gate, but Reload and Revert still read the
active file path directly from disk. A file that started small and later grew
large could therefore bypass the open-file guard through a normal refresh
command.

- **IDE note:** Reload and Revert now check active-file metadata before reading
  and convert files above 16 MiB to the same read-only large-file preview used
  by Open File. Clean matching duplicate tabs are refreshed to the same preview
  without loading the oversized payload.
- **Language note:** no compiler gap surfaced. This is editor reload
  resource-bound hygiene in Rust-owned tab and command routing code.

## L1211 - Persisted Config Text Needs A Read Budget

Theme/settings config, recent-file lists, zoom, keybindings, user snippets, and
LSP overrides are intended to be small text files. They were loaded with
`std::fs::read_to_string`, so a corrupted config path or accidental large file
could be allocated during startup, panel open, snippet refresh, or language
server lookup.

- **IDE note:** config-side text reads now share a 1 MiB metadata gate and
  treat oversized files as unavailable/corrupt before reading. The best-effort
  startup behavior remains unchanged for normal missing or malformed files.
- **Language note:** no compiler gap surfaced. This is startup/config
  resource-bound hygiene in Rust-owned persistence code.
