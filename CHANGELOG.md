...
- **Peek Definition skips oversized cross-file previews**: cross-file peek now
  checks target metadata and refuses previews above 4 MiB before reading,
  avoiding large-file allocation on the navigation path.
...
- **Native dialog output is byte-capped**: Windows PowerShell-backed file,
  folder, save, and project pickers now stream dialog stdout/stderr through
  64 KiB per-stream caps before accepting the selected path.
...
- **Formatter output is byte-capped**: `mty fmt` stdout/stderr now stream
  through 4 MiB per-stream caps, preserving normal failure messages while
  rejecting oversized formatter output.
...
- **Agents inspect output is byte-capped**: `mty inspect --json` stdout/stderr
  now stream through 4 MiB per-stream caps and report live inspect as
  unavailable instead of allocating unbounded runtime JSON or error output.
- **Diagnostics output is byte-capped**: `mty check` stdout/stderr now stream
  through 4 MiB per-stream caps and report `Diagnostics failed: mty check output
  too large` instead of allocating unbounded compiler output.
- **Source Control git output is byte-capped**: push, pull, fetch, and branch
  actions now stream git stdout/stderr through 2 MiB per-stream caps and report
  `git output too large` instead of allocating unbounded process output.
- **Clipboard reads are byte-capped**: editor paste, terminal paste, and snippet
  clipboard-variable expansion now reject clipboard text above 4 MiB before it
  can be inserted into editor or terminal surfaces.
- **Mighty LSP output caps fail closed**: completion, hover/definition, and
  language-action clients now discard oversized `mty lsp` response buffers
  instead of parsing partial response data after crossing their byte budget.
- **Generic LSP output caps fail closed**: external language-server stdout
  streams now discard oversized response or diagnostic buffers instead of
  parsing partial data after crossing the configured byte budget.
- **DAP frames are size-capped before allocation**: inbound debugger
  `Content-Length` frames above 16 MiB are rejected as invalid data, so broken
  adapters cannot reserve enormous buffers before sending JSON.
- **Git blame timezone offsets require digit boundaries**: porcelain
  `author-tz` offsets now require four ASCII digits after the sign, so embedded
  signs such as `++000` or `+00+0` cannot alter cached blame dates.
- **Run output clickable locations require token boundaries**: streamed
  diagnostic location rows now accept only complete unsigned decimal
  line/column tokens, so plus-prefixed or overflowing output cannot create
  bogus jump targets.
- **Compiler diagnostic locations require token boundaries**: `mty check`
  location lines now accept only complete unsigned decimal line/column tokens,
  so plus-prefixed or overflowing reports cannot drive bogus underline or jump
  positions.
- **Git blame timestamps require token boundaries**: porcelain `author-time`
  headers now accept only unsigned decimal timestamp tokens that fit `i64`, so
  plus-prefixed, signed, suffixed, or overflowing blame metadata cannot replace
  cached commit dates.
...
- **Markdown ordered-list overflow preserves structure**: oversized
  ...
  exceeds `u64`.
- **Web Playground port overrides require token boundaries**:
  `MIGHTY_WEB_PORT` now uses the same positive decimal port validation as
  scraped local server URLs, so plus-prefixed, zero, overflowing, or partial
  port values fall back to the default instead of driving malformed launches.
...
- **Terminal CSI/OSC numeric controls require token boundaries**: terminal
  ...
  parameters as valid terminal controls.
- **Headless environment numeric controls require token boundaries**:
  screenshot dimensions and target frames, visible-surface caps,
  lightbulb/settings auto-open rows, and headless frame caps now reject
  plus-prefixed or partial numeric tokens instead of accepting malformed
  automation values.
- **Completion numeric fields reject plus-prefixed tokens**: LSP completion
  ...
  cannot claim function styling, snippet flattening, or replacement ranges.
- **Run output labels require positive decimal coordinates**: compact labels for
  streamed Run diagnostics now reject signed-looking, zero, fractional, or
  overflowing `line:column` tokens instead of presenting malformed output as a
  clickable-looking source location.
- **Snippet indexes reject overflow**: tab-stop numbers and transform capture
  indexes now use checked parsing, so malformed imported snippets cannot create
  saturated tab-stop IDs or capture references.
...
- **Outline document-symbol numbers reject overflow**: LSP document-symbol
  ...
  or impossible outline destinations.
- **DAP payload integers reject overflow**: debugger response sequence IDs,
  stopped-thread IDs, thread IDs, stack-frame IDs/lines, and exit codes now
  reject overflowing numeric tokens instead of saturating them into impossible
  debug state.
- **Agents inspect counters reject overflow**: live `mty inspect --json`
  worker counts, agent IDs, mailbox depths, and high-water counters now reject
  overflowing numeric tokens instead of saturating them into impossible rows.
...
- **LSP payload integers reject overflow**: signature-help indexes,
  parameter-offset labels, and workspace-edit ranges now reject unsigned values
  that do not fit the shim's coordinate type instead of saturating to extreme
  positions that later editor code would clamp.
- **LSP diagnostic severities fail soft**: malformed, missing, or unsupported
  diagnostic `severity` values now fall back to warning presentation instead of
  escalating questionable language-server payloads into editor errors.
- **Web Playground only opens local served URLs**: serve-output scraping now
  latches only loopback `http` / `https` URLs with a positive decimal port, so
  docs links or malformed URL tokens in build output cannot trigger browser
  auto-open.
- **Inline diff hunk headers validate range counts**: malformed unified-diff
  ranges such as negative starts, bad counts, or extra count fields are skipped
  instead of creating bogus line numbers or patchable hunk actions.
- **DAP framing rejects malformed content lengths**: debugger message reads now
  require a positive unsigned decimal `Content-Length`, so malformed headers
  cannot be treated as empty frames or silently desynchronize debug protocol IO.
- **Source-control divergence counts reject signed values**: git branch headers
  now accept only unsigned decimal `ahead` / `behind` counts, so malformed
  porcelain such as `ahead -2` or `behind 1e0` cannot surface bogus sync state.
...
- **Completion deprecated tags reject numeric prefixes**: semantic autocomplete
  ...
  cannot mark fresh rows as deprecated.
- **Agents inspect counters reject numeric prefixes**: live `mty inspect --json`
  worker counts, agent IDs, mailbox depths, and mailbox high-water counters now
  require complete integer tokens instead of truncating malformed numeric
  prefixes into plausible Agents rows.
- **DAP payload integers reject numeric prefixes**: debugger response
  `request_seq`, stopped-thread IDs, thread IDs, stack-frame IDs/lines, and
  exit codes now require complete integer tokens instead of truncating
  fractional numeric prefixes.
- **LSP payload coordinates reject numeric prefixes**: signature-help active
  indexes, parameter-label offset pairs, workspace-edit ranges, and generic
  diagnostics ranges now require complete unsigned integer tokens instead of
  truncating fractional numeric prefixes.
- **Mighty LSP response waits reject numeric prefixes**: the shared
  Mighty-specific hover, definition, completion, signature-help, rename,
  code-action, and outline response scanner now rejects fractional or otherwise
  non-delimited numeric IDs before matching response `id:2`.
- **LSP JSON-RPC IDs reject numeric prefixes**: generic response waiting and
  execute-command apply-edit acknowledgements now reject fractional or otherwise
  non-delimited numeric IDs instead of truncating them to an integer prefix.
- **Execute-command apply-edit handshakes skip malformed requests**: malformed
  `workspace/applyEdit` objects without a usable request ID are now ignored so a
  later valid apply-edit request can still be acknowledged during command
  execution.
...
- **Mighty LSP deep-language capabilities match parsed metadata**: the
  ...
  trigger-context, resolve, or change-annotation flows.
- **LSP outline advertises parsed document-symbol shapes**: generic and
  Mighty-specific language clients now declare hierarchical document-symbol
  support and SymbolKind values, matching the Outline parser without claiming
  unsupported label/tag metadata.
- **LSP rename advertises prepare support**: the generic multi-language client
  and Mighty deep-language client now declare rename prepare support, matching
  the existing `prepareRename` request and parser without claiming change
  annotation support.
- **Generic LSP signature help advertises supported metadata**: the
  registry-backed client now declares markdown/plaintext signature
  documentation and parameter label offset support, while intentionally leaving
  trigger-context support unclaimed.
- **Generic LSP code actions advertise supported metadata**: the
  registry-backed client now declares code-action literal kinds, preferred
  action support, and disabled-action support, while intentionally leaving
  resolve/data support unclaimed.
- **Code Actions honor preferred quick fixes**: LSP `CodeAction.isPreferred`
  is now preserved, the quick-fix menu initially selects the first preferred
  actionable row, and preferred fixes show a distinct row marker.
- **Generic LSP clients advertise protocol parity**: the registry-backed
  multi-language client now advertises the same rich completion item support as
  the completion-only path, and the Mighty hover/definition client advertises
  markdown/plaintext hover content plus definition link support instead of an
  empty capability set.
- **Generic LSP completion advertises supported item capabilities**: the
  completion handshake now declares snippet, commit-character, deprecated,
  preselect, label-details, insert/replace edit, kind, documentation, and list
  default support to servers, and semantic completion honors `textEditText`
  when a completion list supplies the replacement range.
- **Generic LSP completion honors item defaults**: semantic completion now
  applies `CompletionList.itemDefaults.commitCharacters` and safe
  `itemDefaults.editRange` spans when individual items omit those fields, while
  preserving per-item overrides.
...
- **Generic LSP completion honors commit characters**: semantic completion now
  ...
  that character is inserted.
- **Generic LSP completion honors provider preselection**: semantic completion
...
- **Generic LSP completion honors provider ranking**: semantic completion now
  ...
  server order.
- **Generic LSP completion shows provider detail**: semantic completion rows now
  preserve `CompletionItem.detail` and use it for row/footer detail text instead
  of showing placeholder signatures for every server-provided candidate.
- **Generic LSP completion separates display and insert text**: semantic
  completion rows now show the server's `label` when `insertText` or
  `textEdit.newText` differs, while accepting the row still inserts the edit
  text. Snippet and text-edit completions no longer have to display their full
  inserted body in the dropdown.
- **Generic LSP completion honors filter text**: semantic completion candidates
  now preserve LSP `filterText` for prefix matching while still inserting the
  server's `textEdit.newText`, `insertText`, or label text. This keeps aliases
  and abbreviated server matches visible without changing the scalar accept ABI.
...
- **Imported snippet transforms support multiline and dotall regex options**:
  ...
  can be quoted, wrapped, or collapsed without losing imported snippet intent.
- **Imported snippet transforms support conditional replacements**: VS Code
  snippet replacement formats such as `${1:+text}`, `${1:-fallback}`,
  `${1:?yes:no}`, and `${1:fallback}` now branch on whether a regex capture
  matched, improving copied snippet packs that derive optional names.
...
- **Imported snippets resolve random and UUID variables**: VS Code snippets using
  ...
  shapes.
- **Imported snippets resolve Unix timestamp variables**: VS Code snippets using
  `$CURRENT_MILLISECOND`, `$CURRENT_SECONDS_UNIX`, or
  `$CURRENT_MILLISECONDS_UNIX` now expand those values instead of preserving the
  raw marker text, with deterministic timestamp formatting coverage.
...
- **Command Palette unavailable states coalesce with owning workflows**: broken
  ...
  feedback in their Run/Test/Debug/Web/Format/Agents/New Project lanes.
...
- **Save All duplicate-skip messaging avoids a panic edge**: the runtime Save
  ...
  exact filename-specific message for normal duplicate dirty tabs.
...
- **Autocomplete cleanup no longer leaks close-command toasts**: incidental
  ...
  still reports `No autocomplete suggestions open` when appropriate.
