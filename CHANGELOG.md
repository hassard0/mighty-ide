...
- **Tab switches close stale language popups**: active-file sync now also clears
  hover, definition, signature-help, completion, rename, code-action, and
  quick-fix lightbulb state, closes inline peek cards, and dismisses breadcrumb
  menus, so transient language UI cannot follow the wrong tab.
