...
- LSP payload numeric fields for signature help, parameter offset labels,
  ...
  unsigned integer tokens, and the Mighty-specific language parser rejects
  overflow instead of saturating extreme values into clamped signature indexes
  or workspace-edit coordinates
...
- Outline document-symbol error codes, SymbolKind values, and range start lines
  require complete integer tokens, so malformed LSP outline payloads cannot
  trigger scanner fallback, classify rows, or jump to truncated line numbers
- Debugger Adapter Protocol numeric fields for response sequence IDs, stopped
  ...
  tokens, so malformed fractional debugger payloads cannot redirect stack or
  thread state
...
- Agents live-inspect worker counts, agent IDs, mailbox depths, and mailbox
  ...
  snapshots cannot truncate numeric prefixes into plausible rows
