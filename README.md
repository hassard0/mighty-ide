...
### Workspace & UX
- **Explicit Workspace + Open Folder (Ctrl+Shift+O)** — native folder picker (typed-path fallback only when the picker is unavailable) re-roots the file tree, Quick-Open, Search, git, and Agents; **New Folder** (Ctrl+Shift+N) creates workspace directories; active files can be revealed in the IDE file tree or the OS file manager from the command palette; **Recent Folders** (MRU) persist across restarts; explorer header shows the active workspace
